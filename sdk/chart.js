(() => {
  const DistributionDomain = Object.freeze({
    FINITE_AB: 1,   // [a,b]
    A_INF: 2,       // [a, +inf)
    NINF_B: 3,      // (-inf, b]
    NINF_INF: 4     // (-inf, +inf)
  ...
  function inSupport(x, mode, a, b) {
    switch (mode) {
      case DistributionDomain.FINITE_AB: return x >= a && x <= b;
      case DistributionDomain.A_INF: return x >= a;
      case DistributionDomain.NINF_B: return x <= b;
      case DistributionDomain.NINF_INF: return true;
      default: return true;
    }
  }
  ...
  function evalExtended(kind, x, mode, a, b, fn) {
    ...
    switch (mode) {
      case DistributionDomain.FINITE_AB:
        if (x < a) return 0;
        if (x > b) return 1;
        return fn(x);
      case DistributionDomain.A_INF:
        if (x < a) return 0;
        return fn(x);
      case DistributionDomain.NINF_B:
        if (x > b) return 1;
        return fn(x);
      case DistributionDomain.NINF_INF:
        return fn(x);
      default:
        return fn(x);
    }
  ...
  window.DistributionDomain = DistributionDomain;
  const chartFunctions = window.CommonDistributions || window.ChartFunctions || Object.freeze({});
